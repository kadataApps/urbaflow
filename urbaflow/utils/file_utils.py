...
def concat_files(files, sep=';'):
    frames  = [ pd.read_csv(f, sep=sep, encoding=get_encoding_type(f), low_memory=False) for f in files]
    return pd.concat(frames, ignore_index=True)

def concat_files_with_encoding(files, encoding='latin-1'):
    frames  = [ pd.read_csv(f, sep=';', encoding=encoding, low_memory=False) for f in files]
    return pd.concat(frames, ignore_index=True)
