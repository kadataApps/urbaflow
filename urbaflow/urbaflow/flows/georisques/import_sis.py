...
# Données Géorisques SIS (Secteurs d'Information sur les Sols)
# API Web service: https://www.georisques.gouv.fr/webappReport/ws/infosols/resultats/recherche?type=classification&statut=SIS&codeDepartement=85

logger = get_logger(__name__)


@task
def create_table_sis(
    schema: str = "public", table_name: str = "risques_sis", recreate: bool = True
):
    """
    Crée la table PostGIS pour les données SIS (Secteurs d'Information sur les Sols).
    """
    e = create_engine()
...
def parse_sis_item(item: dict, default_department: str | None = None) -> dict:
    """
    Transforme un objet JSON SIS renvoyé par l'API Géorisques en dictionnaire à plat.
    """
    ...
    statut = statut_dto.get("label") or "SIS"
...
@task
def load_sis(
    records: list[dict], schema: str = "public", table_name: str = "risques_sis"
):
    """
    Insère la liste des enregistrements SIS dans la table PostGIS spécifiée.
    """
    if not records:
        logger.info("Aucun enregistrement SIS à insérer.")
        return
...
def add_geometry_column_to_table(
    schema: str = "public", table_name: str = "risques_sis"
):
    """
...
@task
def populate_geom(schema: str = "public", table_name: str = "risques_sis"):
    """
...
@task
def fetch_sis_data(department: str) -> list[dict]:
    """
    Récupère la totalité des données SIS via l'API Web service de Géorisques
    pour un département.
    """
    all_items = []
    page = 0
    ...
    while True:
        url = (
            ...
            f"?type=classification&statut=SIS&codeDepartement={department}&page={page}&size={size}"
        )
        logger.info(
            "Téléchargement de la page %d pour le département SIS %s", page, department
        )
        response = requests.get(url)
        if response.status_code != 200:
            logger.error(
                "Erreur lors du téléchargement des données SIS (statut %d)",
    ...
    logger.info("Total d'éléments SIS récupérés : %d", len(all_items))
    return [parse_sis_item(item, default_department=department) for item in all_items]


@task
def import_sis_files(
    path: Path, schema: str = "public", table_name: str = "risques_sis"
):
    """
    Importe les fichiers JSON SIS locaux présents dans un répertoire.
    """
    files = list_files_at_path(path, r".*sis.*|.*result.*", extension=".json")
    if not files:
    ...
    for file in files:
        ...
        with open(complete_file_path, encoding="utf-8") as f:
            ...
            for item in items:
                all_records.append(parse_sis_item(item))

    load_sis(all_records, schema=schema, table_name=table_name)


@flow
def import_risques_sis_flow(
    path: Path | None = None,
    department: str | None = None,
    db_schema: str = "public",
    table_name: str = "risques_sis",
    recreate: bool = False,
):
    """
    Flow d'importation des données SIS (Secteurs d'Information sur les Sols).
    ...
    create_table_sis(schema=db_schema, table_name=table_name, recreate=recreate)
    add_geometry_column_to_table(schema=db_schema, table_name=table_name)

    if path is None:
        logger.info(
            "Téléchargement et importation des données SIS pour le département %s",
            department,
        )
        target_dir = TEMP_DIR / "georisques/sis" / department
        target_dir.mkdir(parents=True, exist_ok=True)
        target_file = target_dir / f"sis_{department}.json"

        records = fetch_sis_data(department)
        with open(target_file, "w", encoding="utf-8") as f:
            json.dump(records, f, ensure_ascii=False, indent=2)

        load_sis(records, schema=db_schema, table_name=table_name)
    else:
        logger.info("Importation des fichiers SIS depuis le répertoire %s", path)
        import_sis_files(path, schema=db_schema, table_name=table_name)
