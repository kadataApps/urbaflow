...
def transform_before_load_geosirene_etablissement(data: pd.DataFrame, dep: str):
    ...
    columns_to_clean = ['complement_adresse_etablissement',
...
def load_geosirene_etablissement(data: pd.DataFrame):
    e = create_engine()
    chunk_size = 10000 
    logger.info(f"Loading geosirene data to database. {len(data)} rows, {- (-len(data) // chunk_size)+1} chunks")
    data.to_sql("geosirene_etablissement", con=e, if_exists="append", index=False, method="multi",chunksize=chunk_size)
